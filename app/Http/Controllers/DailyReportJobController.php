...
class DailyReportJobController extends Controller
{
    ...
    function saveDailyReport(Request $request) {
        ...
        if($request->daily_report_id) {
            $result = M_DailyJobReport::find($request->daily_report_id)->update($request->except('code_job', 'qty_job', 'code_material', 'qty_material') + [
                'project_name' => ucwords($request->project_name),
            ]);
            return redirect('/job-daily-report/');
        } else {
            $result = M_DailyJobReport::create($request->except('code_job', 'qty_job', 'code_material', 'qty_material') + [
                'project_name' => ucwords($request->project_name),
            ]);
